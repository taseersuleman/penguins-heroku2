...
def Inputt_seq():
    ...
    f = open("Dataset_for_filtering.txt", 'r')
    count = []
    abc = []
    abc = f.read()
    count = []
    ...
    load_model = pickle.load(open('Dhupred_model.pkl', 'rb'))
    pred = load_model.predict(W)
    output_proba = load_model.predict_proba(W)[:, 1]
    # print(output_proba[0])
    # print(output_proba[1])
    lno = len(output_proba)
    # print(lno)
    for i in range(lno):
        #for seq_record in SeqIO.parse("Data_for_Test.txt", "fasta"):
         #   seq = seq_record.seq

        #st.info('Sequence = ' + seq)
        st.subheader("Site Number = ")
        st.write(count[i])
        st.subheader("Sequence")
        st.write(keeper[i])
        # v = 2
        # print(output_proba[i])
        if output_proba[i] > 0.5:
            st.info("Output = Dihydrouridine Site")
        else:
            st.info("Output = Non-Dihydrouridine Site")
...
def input__sequence():
    ...
    if st.sidebar.button("Submit-"):
        f = open("Dataset_for_filtering.txt", 'w')
        f.write(sequence1)
        f.close()
        Inputt_seq()
